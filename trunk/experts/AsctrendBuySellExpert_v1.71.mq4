   ...
   for (int i=0; i<OrdersTotal(); i++) { 
      if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) { 
         if (OrderSymbol()==Symbol() && OrderMagicNumber()==MAGIC) { 
            ...
            else ts = Bid-trailingStop*_Point;    
            ts = NormalizeDouble(ts,Digits);
               if(ts <  Bid && ts > Bid - minstop*_Point) {ts = Bid - minstop*_Point;}
   ...
   for (int i=0; i<OrdersTotal(); i++) { 
      if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) { 
         if (OrderSymbol()==Symbol() && OrderMagicNumber()==MAGIC) { 
            ...
            double minstop = MarketInfo(Symbol(),MODE_STOPLEVEL)+5; 
            if(TrailingMode > 0) double ts = trailingStop;
            else ts = Ask+trailingStop*_Point;       
            ts = NormalizeDouble(ts,Digits);
               if(ts > Ask && ts < Ask + minstop*_Point) {ts = Ask + minstop*_Point;}
...
void ModifyStopLoss(double ldStopLoss) { 
   bool fm;
   fm = OrderModify(OrderTicket(),NormalizeDouble(OrderOpenPrice(),Digits),NormalizeDouble(ldStopLoss,Digits),NormalizeDouble(OrderTakeProfit(),Digits),0,CLR_NONE); 
...
double GetStopLossBuy() { 	return (Bid-lStopLoss*_Point);} 
double GetStopLossSell() { 	return(Ask+sStopLoss*_Point); } 
double GetTakeProfitBuy() { 	return(Ask+lTakeProfit*_Point); } 
double GetTakeProfitSell() { 	return(Bid-sTakeProfit*_Point); }
...
int PartClose()   
{
   ...
   if (OrderMagicNumber() != MAGIC && OrderSymbol() != Symbol()) continue; 
      if (OrderType()==OP_BUY)
      Profit[i] = (MarketInfo(Symbol(),MODE_BID) - OrderOpenPrice())/_Point;
      else  
      if (OrderType()==OP_SELL) 
      Profit[i] = (OrderOpenPrice() - MarketInfo(Symbol(),MODE_ASK))/_Point;
   }
   ...
   if(StringSubstr(OrderComment(),0,5)=="split") continue;
      ...
      if(PartCloseTarget == 0)
      {
         ...
         Target = NormalizeDouble(PctLots*MathAbs(OrderTakeProfit() - OrderOpenPrice())/_Point,Digits);
      }   
